    ...
    return (
      <GlobalStateContext.Consumer>
        {(globalState) => { 
          ...
          const noUserElement = <p className="is-6 has-text-danger subtitle">Download our <a target={`${latestClientReleaseURL ? '_blank' : ''}`} href={`${latestClientReleaseURL || '#'}`}>Automatic Mod Installer</a>, or turn it on to <Link to="/second">supercharge</Link> the website!</p>;
